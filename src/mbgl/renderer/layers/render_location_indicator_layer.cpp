...
//#define MBGL_RENDERLOCATIONINDICATOR_DEBUG 1
...
#if !defined(MBGL_RENDERLOCATIONINDICATOR_DEBUG)
...
#ifdef MBGL_RENDERLOCATIONINDICATOR_DEBUG
...
#ifdef MBGL_RENDERLOCATIONINDICATOR_DEBUG
    {
        const std::array<double, 3> pos2{35.693055, 139.766707, 0};
        renderImpl->parameters.puckPosition = LatLng{pos2[0], pos2[1]};
        renderImpl->parameters.errorRadiusColor = premultiply({1, 0, 0, 0.2});
        renderImpl->parameters.errorRadiusBorderColor = premultiply({1, 0, 1, 0.6});
        renderImpl->parameters.errorRadiusMeters = 90.0f;
    }
#endif
